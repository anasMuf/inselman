        ...
        <div class="card">
            <div class="card-body">
                <div class="mb-3 d-flex flex-wrap justify-content-between">
                    <h4 class="card-title mb-3">Data Gudang</h4>
    ...
    $(document).ready(function () {
        ...
        var table = $('#tableData').DataTable({
            ...
            columns: [
                ...
                {data: 'address', name: 'address', className: 'text-right'},
                {data: 'contact', name: 'contact', className: 'text-right'},
...
<script>
    ...
    function form_data(id=nul,action='edit') {
        ...
        $('.preload-page').show()
        if(action == 'edit'){
            $.get("{{ route('getDataWarehouse') }}",{id:id})
            .done(function(result){
                if(result.status == 'success'){
                    ...
                    $('#form-title').text('Edit Data Warehouse')
                    $('.forms-warehouse form-control').removeAttr('disabled')
            ...
            $.get("{{ route('formContentWarehouse') }}")
            .done(function(result){
                $('.preload-page').hide()
                $('.other-page').show()
                $('.other-page').html(result.content)
                $('#form-title').text('Tambah Data Warehouse')
                $('.forms-warehouse form-control').removeAttr('disabled')
            ...
            .done(function(result){
                if(result.status == 'success'){
                    ...
                    $('.other-page').show()
                    $('.other-page').html(result.content)
                    $('#form-title').text('Detail Data Warehouse')
                    $('.forms-warehouse .form-control').prop('disabled',true)
                    $('.btn-simpan').hide()
    ...
    function form_save() {
        $('.forms-warehouse').submit(function(e){
            e.preventDefault()
